...
fig.add_trace(go.Scatter(x= x_values, y= df_2(x_values), mode='lines', name='h=0.25', line=dict(color='royalblue', width=1)))
...
fig.add_trace(go.Scatter(x= x_values, y= ddf_2(x_values), mode='lines', name='h=0.25', line=dict(color='royalblue', width=1)))
fig.add_trace(go.Scatter(x= x_values, y= ddf_1(x_values), mode='lines', name='h=0.1', line=dict(color='teal', width=1)))
...
plt.figure(figsize=(8,6))
plt.plot(x_values,f(x_values), label="Función")
plt.plot(x[puntos_in], y[puntos_in], 'o',  color="red", label = "Puntos in", alpha =0.05)
plt.plot(x[~puntos_in], y[~puntos_in], 'o',  color="blue", label = "Puntos out", alpha =0.05)
plt.grid()
plt.axis("square")
plt.legend()
plt.show()
...
resultado, error = integrate.dblquad(integrand, -1, 1, lambda y: 1, lambda y: 2)
...
resultado, error = integrate.dblquad(integrand, 0, 6, lambda y: 1, lambda y: 5)

resultado, error
```

c)  REVISAR estructura de la integral
Razón: Se integra primero respecto a x pero el límite superior es una función de x (log(x)), lo que creo no tiene sentido dado que deberían de ser limites en funciones de y.
...
#resultado, error = integrate.dblquad(integrand, 1, np.exp(x), lambda x: 0, lambda x: np.log(y))
...
resultado, error = integrate.dblquad(integrand, 0, 4, lambda x: 0.25*x, lambda x: x)
...
resultado, error = integrate.tplquad(integrand, 
0, 1, #Límites x
...
print(f"Media: {mean}, Varianza: {var}, Sesgo: {skew}, Kurt: {kurt}")


x = np.linspace(fisk.ppf(0.01, c), fisk.ppf(0.99, c), 100)
ax.plot(x, fisk.pdf(x, c),
'r-', lw=2, label='fisk pdf (c=5)')

rv = fisk(c)
ax.plot(x, rv.pdf(x), 'k--', lw=2, label='frozen pdf')

#datos aleatorios
r = fisk.rvs(c, size=500)
ax.hist(r, density=True, bins='auto', histtype='stepfilled', alpha=0.2)
ax.set_xlim([x[0], x[-1]])
ax.legend(loc='best', frameon=False)
plt.title("Distribución Fisk (c=5)")
plt.show()


# Cálculo de probabilidad entre 2 y 4 con cdf
p_cdf = fisk.cdf(4, c) - fisk.cdf(2, c)
print(f"Probabilidad usando CDF P(2 < X < 4): {p_cdf:.4f}")

#Calculo de probabilidad integrando
p_integral, _ = quad(fisk.pdf, 2, 4, args=(c,))
print(f"Probabilidad usando integración de la PDF: {p_integral:.4f}")
